...
public class WalletStripTests
{
    ...
    public void APinnedCurrencyShowsEvenAtZero()
    {
        ...
        var rows = WalletStrip.Pick([new Holding(Orange, 0, 4_000)], pinned: id => id == Orange.Id);
    ...
    {
        var rows = WalletStrip.Pick([new Holding(Orange, 12, 4_000)], pinned: id => id == Orange.Id);
    ...
    public void AnUnpinnedCurrencyHalfwayToItsCapIsNoise()
    {
        ...
        var rows = WalletStrip.Pick([new Holding(Seals, 3_130, 4_000)], pinned: _ => false);
    ...
    {
        var rows = WalletStrip.Pick([new Holding(Seals, 3_600, 4_000)], pinned: _ => false);
    ...
    {
        var rows = WalletStrip.Pick([new Holding(Seals, 1_000_000, null)], pinned: _ => false);
    ...
    {
        var rows = WalletStrip.Pick([new Holding(Orange, 3_999, 4_000)], pinned: id => id == Orange.Id);
    ...
    [Fact]
    public void PinnedComeFirstInTheirOwnOrderThenWarningsWorstFirst()
    {
        ...
        var rows = WalletStrip.Pick(
            ...
            pinned: id => id == Orange.Id || id == Purple.Id);
