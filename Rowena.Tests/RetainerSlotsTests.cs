...
{
    /// <summary>A candidate as the pile hands one over: its realised share worked out already.</summary>
    private static SlotCandidate Candidate(uint itemId, int units, long worth, double? days) =>
        new(itemId, units, worth, RetainerSlots.Realised(worth, days, 7));

    [Fact]
    public void AStackThatClearsInsideTheHorizonIsWorthAllOfIt()
    {
        var picks = RetainerSlots.Fill([Candidate(1, 5, 10_000, 2)], slots: 1);
    ...
    public void AStackThatTakesMonthsIsWorthTheSliceThatSells()
    {
        ...
        var picks = RetainerSlots.Fill([Candidate(1, 415, 120_765, 116)], slots: 1);
    ...
    public void TheSlowFortuneLosesTheSlotToTheQuickerSmallerOne()
    {
        ...
        var picks = RetainerSlots.Fill(
            [Candidate(1, 415, 120_765, 116), Candidate(2, 147, 46_452, 0.3)],
            slots: 1);
    ...
    {
        var picks = RetainerSlots.Fill(
            [Candidate(1, 10, 999_999, null), Candidate(2, 1, 10, 1)],
            slots: 4);
    ...
    {
        var picks = RetainerSlots.Fill(
            [Candidate(1, 1, 500, 1), Candidate(2, 1, 400, 1), Candidate(3, 1, 300, 1)],
            slots: 2);
    ...
    public void NoSlotsIsNoPlan() =>
        Assert.Empty(RetainerSlots.Fill([Candidate(1, 1, 500, 1)], slots: 0));

    [Fact]
    public void AWorthlessStackIsNotWorthASlotEither() =>
        Assert.Empty(RetainerSlots.Fill([Candidate(1, 10, 0, 1)], slots: 5));
